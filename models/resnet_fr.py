...
class CifarResNetSplitBlock(nn.Module):
  def __init__(self, model, layers, splits_id, num_splits, delay):
    super(CifarResNetSplitBlock, self).__init__()
...
def resnet_fr(depth, num_classes=10, num_splits=2):
  ...
  for splits_id in range(num_splits):
    ...
    net = CifarResNetSplitBlock(model, model.layers[left_idx:right_idx], splits_id, num_splits, num_splits-1-splits_id)
    nets.append(net) 
